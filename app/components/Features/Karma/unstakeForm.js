...
const FormObject = props => {
  ...
  const formProps = {
    handleSubmit,
    submitColor: 'rose',
    submitText: intl.formatMessage(messages.karmaUnstakeFormSubmitText),
  };
  const FormData = [
    {
      id: 'owner',
...
const StakeForm = props => {
  ...
  return (
    <ToolBody
      color="warning"
      icon={Undo}
      header={intl.formatMessage(messages.karmaUnstakeFormHeader)}
      subheader={intl.formatMessage(messages.karmaUnstakeFormSubHeader)}>
      <FormObject {...props} />
    </ToolBody>
  );
};
